...
class ListUsersCommand extends Command
{
    ...
    {
        $this
            ->setName('security:list-users')
            ->setDescription('Lists all the existing users')
            ->setAliases(['security:users'])
            ->setHelp(
                <<<'HELP'
                ...
                the email address specified in the <comment>--send-to</comment> option:
                  ...
                  <info>php %command.full_name%</info> <comment>--send-to=hello@biurad.com</comment>
                HELP
            )
    ...
    private function sendReport(string $contents, string $recipient): void
    {
        $email = (new Email())
            ...
            ->to($recipient)
            ->subject(\sprintf('security:list-users report (%s)', \date('Y-m-d H:i:s')))
            ->text($contents);
